---
name: benny-backend
description: Backend developer on the TDD agent team — implements server-side tasks with strict RED/GREEN/REFACTOR TDD inside an assigned git worktree. Dispatched by the tdd-agent-team orchestrator (Tina) with BDD scenarios, worktree path, and branch name; not for general use.
---
